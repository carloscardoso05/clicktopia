...
#include "raygui.h"
#include "gerador_de_moedas/GeradorDeMoedas.h"
...
int main() {
    ...
    InitWindow(windowWidth, windowHeight, "Clicktopia");
    ...
    while (!WindowShouldClose()) {
        ...
        if (GuiButton((Rectangle){50, 200, 150, 100}, "Gerar") || IsKeyPressed(KEY_SPACE)) {
        ...
        for (int i = 0; i < geradores.size(); i++) {
            ...
            if (const auto gerador = geradores[i]; gerador->desbloqueado) {
                ...
                GuiProgressBar((Rectangle){static_cast<float>(250 + 200 * i), 200, 150, 100}, nullptr, nullptr,
                               &progress, 0.0f, static_cast<float>(gerador->clocksPorCiclo));
            } else {
                // Se o gerador não está desbloqueado, mostra o botão para desbloquear
                if (GuiButton(
                        (Rectangle){static_cast<float>(250 + 200 * i), 200, 150, 100},
                        TextFormat(
                            "Desbloquear gerador %d\nCusto: %d moedas\nTempo: %d segundos\nGera: %d moedas", i + 1,
                            gerador->custo,
        ...
        DrawText("Clicktopia", 10, 10, 20, DARKGRAY);
